	...
	public boolean add(Warehouse shipWarehouse, int numberOfConteiners) throws InterruptedException {
		...
		try {
			...
			if (portLock) {
				 ...
				 * новое количество высчитываетс€ как старое + добавка, необходимо сравнивать 
				 * с общим местом в хранилище порта. Ќеобходимо portWarehouse.getSize()*/
	...
	private boolean doMoveFromPort(Warehouse shipWarehouse, int numberOfConteiners) throws InterruptedException {
		...
		try {
			...
			if (shipLock) {
				...
				//TODO 
				/*¬ысчитанное новое количество должно сравниватьс€ не со 
				 * свободным, а с общим местом в хранилище корабл€.
				 * Ќеобходимо shipWarehouse.getSize()*/
